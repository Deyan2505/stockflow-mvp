...
import { type Customer, type CustomerResult, deactivateCustomer, restoreCustomer } from './actions'
import { CustomerModal } from './customer-modal'
import { cn } from '@/lib/utils'
import { useT } from '@/lib/i18n'

export function CustomersClient({ customers, canWrite }: { customers: Customer[]; canWrite: boolean }) {
  const { t } = useT()
  const s = t.customers
  ...
  const handleAction = (promise: Promise<CustomerResult>, msg: string) => {
      ...
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {s.activeCount(customers.filter((x) => x.status === 'active').length)}
          </p>
        </div>
        {canWrite ? (
          <button
                  ...
                  <td className="px-4 py-3">
                    {canWrite && (
                      <div className="flex items-center gap-3">
                        ...
                        {item.status === 'active' ? (
                          <button
                            onClick={() => handleAction(deactivateCustomer(item.id), s.successDeactivate)}
                            disabled={isPending}
                          ...
                          <button
                            onClick={() => handleAction(restoreCustomer(item.id), s.successRestore)}
                            disabled={isPending}
