...
import { type Customer, type CustomerInput, createCustomer, updateCustomer } from './actions'
...
export function CustomerModal({ customer, onClose }: Props) {
  ...
  const [form, setForm] = useState<CustomerInput>(customer ? toInput(customer) : empty)
  const [error, setError] = useState<string | null>(null)
  const [isPending, startTransition] = useTransition()

  const set = (key: keyof CustomerInput, val: string) =>
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      ...
      if (!result.success) {
        setError(result.error === 'errDuplicate' ? s.errDuplicate : result.error)
        return
      }
      onClose(customer ? s.successUpdate : s.successCreate)
    })
  }
      ...
      <div className="w-full max-w-lg rounded-xl bg-white shadow-xl dark:bg-gray-900">
        <div className="flex items-center justify-between border-b border-gray-100 px-6 py-4 dark:border-gray-800">
          <h2 className="font-semibold text-gray-900 dark:text-white">
            {customer ? s.modalEditTitle : s.modalNewTitle}
          </h2>
          <button
            onClick={() => onClose()}
        ...
        <form onSubmit={handleSubmit} className="p-6">
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            >
              {isPending ? s.saving : customer ? s.save : s.create}
            </button>
          </div>
        </form>
      </div>
