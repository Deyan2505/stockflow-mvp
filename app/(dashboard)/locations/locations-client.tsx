  ...
  const filtered = useMemo(() => {
    ...
    return locations.filter((l) => {
      const matchSearch =
        ...
        (l.warehouses?.name ?? '').toLowerCase().includes(q)
      const matchWarehouse =
        warehouseFilter === 'all' || l.warehouse_id === warehouseFilter
      const matchStatus = showInactive || l.status === 'active'
      return matchSearch && matchWarehouse && matchStatus
    })
  }, [locations, search, warehouseFilter, showInactive])

  const columns = ['Код', 'Склад', 'Зона', 'Ред', 'Рафт', 'Кошче', 'Статус', '']

  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Локации</h1>
          <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
            {locations.filter((l) => l.status === 'active').length} активни локации
          </p>
        </div>
        <button
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Търси по код, зона или склад…"
            value={search}
        ...
        >
          <option value="all">Всички складове</option>
          {warehouses.map((w) => (
            ...
            {filtered.length === 0 ? (
                ...
                <td colSpan={columns.length} className="px-4 py-16 text-center">
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    {search || warehouseFilter !== 'all'
                      ? 'Няма намерени локации'
                      : 'Все още няма локации'}
                  </p>
                  {!search && warehouseFilter === 'all' && (
                    <p className="mt-1 text-xs text-gray-300 dark:text-gray-600">
                      Първо добави склад, след това създай локации в него
                    </p>
                  )}
                </td>
              </tr>
              ...
              filtered.map((l) => (
                ...
                >
                  ...
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{l.zone ?? '—'}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{l.row ?? '—'}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{l.shelf ?? '—'}</td>
                  <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{l.bin ?? '—'}</td>
                  <td className="px-4 py-3">
                    ...
                    >
                      {l.status === 'active' ? 'активна' : 'неактивна'}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      {l.status === 'active' ? (
                        ...
                        <button
                          onClick={() => startTransition(() => restoreLocation(l.id))}
                          disabled={isPending}
