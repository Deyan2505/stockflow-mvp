...
export function InventoryClient({ rows, warehouses }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      const matchWarehouse =
        warehouseFilter === 'all' || r.locations?.warehouse_id === warehouseFilter
  ...
  const columns = ['Продукт', 'SKU', 'Склад', 'Локация', 'Ед.мярка', 'Налично', 'Мин.', 'Статус']

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">Наличност</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
          Текущо количество по продукт и локация
        </p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-3 gap-4">
        <div className="rounded-xl border border-gray-100 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
          <p className="text-xs font-medium text-gray-500 dark:text-gray-400">В наличност</p>
          <p className="mt-1 text-2xl font-semibold text-gray-900 dark:text-white">
            {totalWithStock}
          </p>
          <p className="text-xs text-gray-400 dark:text-gray-600">позиции</p>
        </div>
        ...
        >
          <p
            className={cn(
              'text-xs font-medium',
              zeroStock > 0 ? 'text-red-700 dark:text-red-400' : 'text-gray-500 dark:text-gray-400'
            )}
          >
            Изчерпано
          </p>
          <p
            className={cn(
              'mt-1 text-2xl font-semibold',
              zeroStock > 0 ? 'text-red-700 dark:text-red-400' : 'text-gray-900 dark:text-white'
            )}
          >
            {zeroStock}
          </p>
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Търси по продукт или SKU…"
            value={search}
              ...
              <tr>
                <td colSpan={columns.length} className="px-4 py-16 text-center">
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    {rows.length === 0 ? 'Все още няма наличност' : 'Няма намерени позиции'}
                  </p>
                  {rows.length === 0 && (
                    <p className="mt-1 text-xs text-gray-300 dark:text-gray-600">
                      Наличността се обновява автоматично при записване на движения
                    </p>
                  )}
                </td>
              </tr>
              ...
              filtered.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 font-mono text-sm font-medium text-gray-700 dark:text-gray-300">
                      {r.locations?.code ?? '—'}
                    </td>
